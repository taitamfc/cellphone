...
jQuery(function($) 
{
	...
	DupPro.Pack.PopulateCurrentTemplate = function () 
	{
		...
		//	$("#cpnl-dbaction").val(selectedTemplate.installer_opts_cpnl_db_action);
            ...
            var installer_opts_brand = (typeof selectedTemplate.installer_opts_brand != 'undefined' && selectedTemplate.installer_opts_brand > 0 ? Number(selectedTemplate.installer_opts_brand) : <?php echo $active_brand->id; ?>),
                installer_opts_brand_id = [],
                x = 0;

            // find and fix remved brands
            for (var i = 0; i < packageTemplates.length; i++) {
                if(i > 1 && typeof packageTemplates[i].installer_opts_brand != 'undefined' && null != packageTemplates[i].installer_opts_brand)
                {
            ...
            if(!(installer_opts_brand_id.indexOf(installer_opts_brand) > -1))
            {
